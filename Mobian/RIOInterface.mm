...
- (void)startListening:(mbnViewController*)aListener {
					   ...
					   AudioBufferList				*ioData)
{
	...
	uint32_t stride = 1;
	int bufferCapacity = (int)THIS->bufferCapacity;
...
void ConvertInt16ToFloat(RIOInterface* THIS, void *buf, float *outputBuf, size_t capacity) {
	...
	outFormat.mBitsPerChannel = (double)8 * bytesPerSample;
	outFormat.mFramesPerPacket = 1;
	outFormat.mChannelsPerFrame = 1;	
	outFormat.mBytesPerPacket = (double)bytesPerSample * outFormat.mFramesPerPacket;
	outFormat.mBytesPerFrame = (double)bytesPerSample * outFormat.mChannelsPerFrame;
	outFormat.mSampleRate = THIS->sampleRate;
	
	const AudioStreamBasicDescription inFormat = THIS->streamFormat;
	
	UInt32 inSize = (double)capacity*sizeof(SInt16);
	UInt32 outSize = (double)capacity*sizeof(float);
...
- (void)createAUProcessingGraph {
	...
	bufferList->mBuffers[0].mDataByteSize = (double)kBufferSize*bytesPerSample;
...
- (size_t)ASBDForSoundMode {
	...
	asbd.mBitsPerChannel = (double)8 * bytesPerSample;
	asbd.mFramesPerPacket = 1;
	asbd.mChannelsPerFrame = 1;	
	asbd.mBytesPerPacket = (double)bytesPerSample * asbd.mFramesPerPacket;
	asbd.mBytesPerFrame = (double)bytesPerSample * asbd.mChannelsPerFrame;
